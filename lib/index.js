...
function fromFileWithMimeAndPath( type, filePath, options, cb ) {
  ...
  if ( typeof type === 'string' && typeof filePath === 'string' ) {
    if ( typeof cb === 'function' && typeof options === 'object' ) {
      ...
      return _extractWithType( type, filePath, options, cb );
...
function fromBufferWithMime( type, bufferContent, options, cb, withPath ) {
  if ( typeof type === 'string' &&
    ...
    _writeBufferToDisk( bufferContent, function( newPath ) {
      const isOptionsCb = typeof options === 'function';
      const extendedCallback = function (error, success) { 
        try {
          fs.unlinkSync(newPath); 
        } catch(err) {
          console.log(err);  
        }
        isOptionsCb ? options(error,success) : cb(error,success); 
      }; 
      fromFileWithMimeAndPath( type, newPath, isOptionsCb ? extendedCallback : options, isOptionsCb ? cb : extendedCallback );
    });
  } else {
    _returnArgsError( arguments );
  }
